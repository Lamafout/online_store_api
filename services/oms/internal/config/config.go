...
func LoadConfig() (*Config, error) {
	if err := godotenv.Load(); err != nil {
		...
		log.Println("Successfully loaded .env file for OMS")
	}

	user := getEnv("OMS_DB_USER", "")
	password := getEnv("OMS_DB_PASSWORD", "")
	dbName := getEnv("OMS_DB_NAME", "online_store_oms")
	port := getEnv("OMS_DB_PORT", "5432")
	host := getEnv("OMS_DB_HOST", "localhost")

	serverPort := getEnv("OMS_SERVER_PORT", "8080")
	...
	if user == "" || password == "" || dbName == "" || port == "" || host == "" || serverPort == "" {
		return nil, fmt.Errorf("missing required environment variables for OMS")
	}
