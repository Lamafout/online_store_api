...
import (
	...
	uowpkg "github.com/Lamafout/online-store-api/services/oms/internal/dal/unit_of_work"
...
func (s *OrderService) BatchCreateOrders(
	...
	var msgs []any
	for _, order := range orders {
		msg := contracts.OrderCreatedMessage{
			ID:                 order.ID,
			CustomerID:         order.CustomerID,
			DeliveryAddress:    order.DeliveryAddress,
			TotalPriceCents:    order.TotalPriceCents,
		...
		for _, item := range order.Items {
			msg.OrderItems = append(msg.OrderItems, contracts.OrderItemMessage{
				ID:            item.ID,
